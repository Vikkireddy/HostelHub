...
export function SubscriptionGuard({ children }: { children: React.ReactNode }) {
  ...
  const [checkedHostelId, setCheckedHostelId] = useState<number | null>(null);

  const checkSubscription = useCallback(async () => {
    if (!hostelId) return;
    const targetHostelId = hostelId;
    setHasChecked(false);
    setCheckedHostelId(null);
    try {
      const res = await fetchWithHostel("/api/subscription/status", targetHostelId);
      ...
      setHasChecked(true);
      setCheckedHostelId(targetHostelId);
    }
  }, [hostelId, setStatus]);
  ...
  useEffect(() => {
    ...
    if (checkedHostelId !== hostelId) return;
    if (shouldRedirectToSubscription(status)) {
      router.replace(SUBSCRIPTION_PAGE_PATH);
    }
  }, [hostelId, pathname, status, router, hasChecked, checkedHostelId]);
