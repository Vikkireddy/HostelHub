...
type PaymentConfigurationSettingsTabProps = {
  paymentTrackingEnabled: boolean;
  onPaymentTrackingChange: (checked: boolean) => void;
  allowEdit?: boolean;
};

export function PaymentConfigurationSettingsTab({
  paymentTrackingEnabled,
  onPaymentTrackingChange,
  allowEdit = true,
}: PaymentConfigurationSettingsTabProps) {
  return (
    <TabsContent value="payment-configuration" className="mt-0">
      <Card>
        <CardHeader>
        ...
        <CardContent>
          ...
          <Typography variant="muted" className="mt-3 text-xs">
            {t("SETTINGS_CONFIG_PAYMENTS_ROW_HINT")}
          </Typography>
        </CardContent>
      </Card>
    </TabsContent>
